...
export async function onRequestPost({ request, env }) {
  ...
  const sys = ask
    ...
    : concept
      ? ('你面对的是一位脱产多年、基础已经遗忘、正在重新备考的成年学习者。请【不要解这道题】，而是把这道题所涉及的【前置知识点、公式、概念】系统而通俗地讲清楚，帮 TA 补回基础。使用 Markdown，' + fmtRule + '结构：①「涉及的知识点」——列出本题用到的核心概念/定理/公式（3~6 个）；②对每个知识点：用一句话说明它是什么、写出公式、再用最直白的话解释它的直觉含义或几何意义，必要时举一个最简单的例子；③「它们如何用在这类题里」——点明这些知识点在本题中扮演的角色。全程假设读者忘了大部分基础，多用类比、少跳步，不堆术语；不要给出本题的最终答案或完整解题步骤。中文回答，直接开始，不要客套话。')
