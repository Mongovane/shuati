...
export async function onRequestPost({ request, env }) {
  ...
  const sys = ask
    ...
    : concept
      ? ('你面对的是一位脱产多年、基础已经遗忘、正在重新备考的成年学习者。请【不要解这道题】，而是提取这道题所涉及的【前置知识点/公式/概念】，做成 3~6 张知识卡片帮 TA 补基础。\n只输出一个 JSON 数组，不要任何额外文字、不要 Markdown 代码块围栏。数组每个元素形如：\n{"term":"知识点名称","formula":"核心公式（用 $...$ 包裹的 LaTeX，没有公式则留空字符串）","plain":"用最直白的话解释它是什么、直觉或几何含义，假设读者忘光了基础，多用类比","example":"一个最简单的例子或它在本题中的作用（一两句）"}\n要求：term 简短；plain 通俗不堆术语；公式定界符只用 $...$，禁止 \\\\( \\\\) \\\\[ \\\\]；整个输出必须是可被 JSON.parse 解析的合法数组，不要输出答案或完整解题步骤。')
