...
一个跑在 Cloudflare 免费额度上的个人刷题工具，覆盖**全部考试科目**（政治理论 / 英语 / 高等数学 / 大学语文 / 计算机基础与程序设计），支持单选、多选、判断、填空、简答论述、程序设计六种题型。
...
| 科目覆盖 | 偏重专业课 | 五科全覆盖，题型自适应（公式 LaTeX、代码高亮、阅读材料） |
...
{
  "id": "可选，留空自动生成",
  "subject": "politics | english | math | chinese | computer",
...
- **CDN**：前端依赖（Vue / KaTeX / highlight.js / marked）走 `cdnjs.cloudflare.com`（Cloudflare 自家 CDN，国内多数网络可达）；字体用系统字体，无需谷歌字体。若某些网络下加载偏慢，可把这几个库文件下载进 `public/`，再把 `index.html` 顶部的 `<script>`/`<link>` 改成本地相对路径自托管。
...
- 备考：收集近 5 年五科真题（政治、英语、高数/语文、计算机基础与程序设计），每天用 AI 清洗 1~2 套入库。
