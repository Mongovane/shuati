...
async reviewMock(m){ if(!m||!m.id){ this.flash('这次记录没有逐题明细（旧版本考的）',true); return; } try{
        ...
        this.queue=qd.items; this.qi=0; this.queueTotal=qd.items.length; this.batchDone=false; this.loadedOnce=true; this.sessionAns={}; this.sessionStart=Date.now(); this.streak=0;
        // 独立「错题回顾」会话：走 wrong 视图（复习语境），用横幅标明来源，退出前不被常规错题队列覆盖
        this.reviewSession={ title:this.subjName(m.subject)+(m.taken_at?' · '+new Date(m.taken_at*1000).toLocaleDateString():''), count:qd.items.length };
        this.sessionView='wrong'; this.view='wrong'; this.filterLock=true;
        this.flash('已载入该次测试的 '+qd.items.length+' 道错题');
      }catch(e){ if(e.message!=='unauth')this.flash(e.message,true); } },
// 删除一条模考历史（成绩单存档；只删这条记录与其明细，不影响题库/复习进度）
async deleteMock(m){ if(!m||!m.id)return; if(!this.token){ this.flash('请先在设置中填写访问码',true); return; }
      if(!confirm('删除这条测试记录？（只删这次的成绩存档，不影响题库和复习进度）'))return;
      try{ await this.api('/api/progress?mock_id='+m.id,{method:'DELETE'});
        if(this.stats&&Array.isArray(this.stats.mocks)) this.stats.mocks=this.stats.mocks.filter(x=>x.id!==m.id);
        this.flash('已删除该测试记录');
...
async restoreBackup(ev){ const f=ev&&ev.target&&ev.target.files&&ev.target.files[0]; if(!f)return;
      ...
      const tip=mode==='replace'
        ? '覆盖式恢复：会先清空现有的题库/进度/教材/模考等数据，再写入备份内容（恢复到备份时刻）。\n\n'+summary+'\n\n确定继续？'
...
rateQ(r){ const s=r.seen||0; return s?Math.round((r.right_q||0)/s*100):0; },  /* 按题掌握率：做对题数/已作答题数 */
