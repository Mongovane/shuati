...
const TPL_SHELL_OPEN = `
  ...
  <div class="tabs">
    ...
    <button class="tab" :class="{active:view==='settings'}" @click=\"go('settings')\">Settings <span class="muted" style="font-size:10px">{{ appVer }}</span></button>
  </div></div>

  <div v-if="offline" class="offline-bar">离线模式 · 显示已缓存内容，作答将在联网后自动同步<span v-if="offlineQueued>0">（待同步 {{ offlineQueued }} 条）</span></div>

  <div class="wrap" :class="{'has-bottombar':hasBottomBar, 'wrap-wide':['bank','books'].includes(view)}">
`;
