...
const App={
  ...
  data(){ return {
    ...
    stats:null, statsDirty:true, statsLoading:false, bankDirty:true, /* 题库脏标记：首次 true，此后仅题目增删改后置位 */ settFold:{ token:false, aicfg:true, mineru:true, offline:true, subjects:true, prefs:true }, settOrder:['token','mineru','aicfg','offline','subjects','prefs'],
...
onFocus(){ if(this.stealth.autoHide) this.stealth.hidden=false; }
  ...
  mounted(){ try{ window.__hideSplash&&window.__hideSplash(); }catch(_){}
    ...
    try{ const so=JSON.parse(localStorage.getItem('zb_settorder')||'null'); if(Array.isArray(so)&&so.length){ const valid=['token','mineru','aicfg','offline','subjects','prefs']; const merged=so.filter(x=>valid.includes(x)); for(const v of valid)if(!merged.includes(v))merged.push(v); this.settOrder=merged; } }catch(_){}
